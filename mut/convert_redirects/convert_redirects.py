...
def str_represents_version_number(s):
    try:
        int(s[0])
        return True
    except ValueError:
...
def create_pragmas(base: str, versions: [str], symlinks: [str]) -> List[str]:
    ...
    if versions:
        for i, v in enumerate(versions):
            if isinstance(v, str) and str_represents_version_number(v):
                versions[i] = 'v'+v
        ver = ' '.join(
...
def process_rule(rule: dict, base: str, cfg: dict, pragmas: List[str]):
    ...
    for output in rule['outputs']:
        v = None if not defaultdict(None, cfg).get('versions') else cfg['versions']
        ...
        if o.version == 'raw':
            ...
            f = '/${version}'
            t = '${base}/${version}' if not re.match(r'http*', o.old_prefix) else ''

        f += '/'.join([o.old_prefix, rule['from'].lstrip('/')]) if o.old_prefix else rule['from']
